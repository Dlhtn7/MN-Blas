...
void s(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_sgemm (101,111,111,VECSIZE,VECSIZE,VECSIZE,1.0,m1,1,m12,1,1.0,m13,1) ;
     TOP_NANO (end);
...
void d(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     m2_init(m22, 3.0);
     TOP_NANO (start) ;
        mncblas_dgemm (101,111,111,VECSIZE,VECSIZE,VECSIZE,1.0,m2,1,m21,1,1.0,m22,1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
...
void c(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_cgemm (101,111,111,VECSIZE,VECSIZE,VECSIZE,&a,m3,1,m31,1,&b,m32,1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
...
void z(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_zgemm (101,111,111,VECSIZE,VECSIZE,VECSIZE,&a,m4,1,m41,1,&b,m42,1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
