...
void s(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mnblas_saxpy (VECSIZE,1.0, vec1, 1, vec2, 1) ;
     TOP_NANO (end);
...
void d(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mnblas_daxpy (VECSIZE, 1.0,d1, 1, d2, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
...
void c(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mnblas_caxpy(VECSIZE, &a,v1, 1, v2, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
...
void z(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mnblas_zaxpy (VECSIZE, &a, v3, 1, v4, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
