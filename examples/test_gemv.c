...
void s(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_sgemv (101,111,VECSIZE,VECSIZE,1.0,m1,1,vec1,1,1.0,vec2,1) ;
     TOP_NANO (end);
...
void d(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     m2_init(m2, 1.0);
     TOP_NANO (start) ;
        mncblas_dgemv (101,111,VECSIZE,VECSIZE,1.0,m2,1,d1,1,1.0,d2,1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
...
void c(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_cgemv (101,111,VECSIZE,VECSIZE,&a,m3,1,v1,1,&b,v2,1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
...
void z(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_zgemv (101,111,VECSIZE,VECSIZE,&a,m4,1,v3,1,&b,v4,1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
