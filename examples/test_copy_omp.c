...
void d(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_dcopy_omp (VECSIZE, d1, 1, d2, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
...
void c(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_ccopy_omp (VECSIZE, v1, 1, v2, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
...
void z(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mncblas_zcopy_omp (VECSIZE, v3, 1, v4, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
