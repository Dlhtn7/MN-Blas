...
void test_mncblas_caxpy() {
    complexe_float_t X[3];
    complexe_float_t Y[3];
    X[0].real = 1, X[0].imaginary = 1, X[1].real = 2, X[1].imaginary = 2, X[2].real = 3, X[2].imaginary = 3;
    Y[0].real = 1, Y[0].imaginary = 1, Y[1].real = 2, Y[1].imaginary = 2, Y[2].real = 3, Y[2].imaginary = 3;

    int N = 3;
    Voir_complexe_float(X,Y,3);
    complexe_float_t a2 = (complexe_float_t){1.0,1.0};

    mnblas_caxpy_omp(N, &a2, X, 1, Y, 1);
...
void test_mncblas_zaxpy() {
    ...
    complexe_double_t a2 = (complexe_double_t){1.0,1.0};

    mnblas_zaxpy_omp(N, &a2, X, 1, Y, 1);
...
void s(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mnblas_saxpy_omp (VECSIZE,1.0, vec1, 1, vec2, 1) ;
     TOP_NANO (end);
  ...
  printf ("saxpy => nb flottants: %d en %e seconde\n", VECSIZE*2,temp_moy/NB_FOIS) ;
...
void d(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mnblas_daxpy_omp (VECSIZE, 1.0,d1, 1, d2, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
  printf ("daxpy => nb flottants: %d en %e seconde\n", VECSIZE*2,temp_moy/NB_FOIS) ;
...
void c(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mnblas_caxpy_omp(VECSIZE, &a,v1, 1, v2, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
  printf ("caxpy => nb flottants: %d en %e seconde\n", VECSIZE*8,temp_moy/NB_FOIS) ;
...
void z(){
  ...
  for (i = 0 ; i < NB_FOIS; i++){
     ...
     TOP_NANO (start) ;
        mnblas_zaxpy_omp (VECSIZE, &a, v3, 1, v4, 1) ;
     TOP_NANO (end);
     temp_moy = temp_moy+diff_nano(&start,&end);
  }
  printf ("zaxpy => nb flottants: %d en %e seconde\n", VECSIZE*8,temp_moy/NB_FOIS) ;
